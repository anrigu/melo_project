...
from marketsim.old_code.utils.log_multimodal import logmultinomial, logmultinomial_torch
'''
...
def profile_rankings(profile):
    ...
    for a in range(num_actions):
        ...
        if stars >= 0 and bars > 0:
            preceding_profiles += torch.special.binom(stars + bars - 1, stars)
    
    return int(preceding_profiles + 1)
