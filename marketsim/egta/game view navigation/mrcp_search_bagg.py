...
def write_data(data_path, gid, setting, new_data, method='m1'):
	...
	file = os.path.join(data_path, str(gid), 'new_mrcp_{0}_{1}.txt'.format(method, str(setting)))
	f = open(file, 'w')
	for nd in new_data:
...
def write_data2(data_path, gid, setting, new_data, method='m1'):
	#file = os.path.join(data_path, str(gid), 'new2_mrcp_{0}_{1}.txt'.format(method, str(setting)))
	file = os.path.join(data_path, str(gid), 'new_mrcp_{0}_{1}.txt'.format(method, str(setting)))
	f = open(file, 'w')
...
"""
#use profiles from quiesce+dpr and full game regret
...
def three_layer(prof_path, game_path, game_id, setting):
	# bagg-fna
	nxt_setting = {3: 7, 4: 9, 5: 13, 7: 17, 9: 25}
	players = 49

	print('Starting 3 layer.')

	setting2 = nxt_setting[setting]
	# for 5 strategy version
...
#uses profiles from quiesce+dpr that are candidate equilibria and full game regret
...
#uses candidate equilibria from quiesce+dpr that are in the restricted game and full game regret
...
if __name__ == '__main__':
	...
	# new multilevel
	start = time.time()
	# setting is the "last" dpr value we used
	eq_profs = three_layer(args.profpath, args.gamepath, args.game, args.dpr)
	#write_data2(args.profpath, args.game, args.dpr, eq_profs, method='multi_m7')
	write_data2(args.profpath, args.game, args.dpr, eq_profs, method='multi_m8')
	print('Wrote data.')
