...
# Import from our own codebase with proper paths
from marketsim.egta_test.subgame_search.symmetric_game import SymmetricGame
from marketsim.egta_test.subgame_search.utils.log_multimodal import logmultinomial
...
def create_symmetric_game_from_data(raw_data, device="cpu"):
    ...
    min_payoff = raw_payoff_table.min()  
    max_payoff = raw_payoff_table.max()
    ...
    epsilon = 1e-6
    normalized_payoffs = np.clip(normalized_payoffs, epsilon, 1.0)
    
    # Convert to log space
    log_payoffs = np.log(normalized_payoffs)
